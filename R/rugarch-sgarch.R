				...
				hess.eps=1e-4, hess.d=0.1, hess.zero.tol=sqrt(.Machine$double.eps/7e-7), r=4, v=2))
{
	...
	if(convergence == 0){
		...
		arglist$data = data
		fit = .makefitmodel(garchmodel = "sGARCH", f = .sgarchLLH, T = T, m = m, 
...
# SECTION sGARCH LLH
#---------------------------------------------------------------------------------
.sgarchLLH = function(pars, arglist)
{
	# prepare inputs
	data = arglist$data
...
#---------------------------------------------------------------------------------
.sgarchfilter = function(spec, data, out.sample = 0, n.old = NULL, rec.init = 'all')
{
...
.nsgarchforecast = function(ipars, modelinc, idx, mu, omega, mxfi, vxfi, h, epsx, z, data, N, n.ahead)
{
	if(modelinc[15]>0){
...
.sgarchpath1 = function(spec, n.sim = 1000, n.start = 0, m.sim = 1, presigma = NA, prereturns = NA, 
...
.sgarchpath2 = function(spec, n.sim = 1000, n.start = 0, m.sim = 1, presigma = NA, prereturns = NA, 
		preresiduals = NA, rseed = NA, custom.dist = list(name = NA, distfit = NA), mexsimdata = NULL, 
		vexsimdata = NULL)
{
	if(spec@model$modelinc[4]>0){
