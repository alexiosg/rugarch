	...
	if(use.solver){
		...
		if(!is.null(sol$par)){
			...
			if(modelinc[7]==0){
				...
				tmpx = .gjrgarchLLH(sol$par, arglist)
...
#---------------------------------------------------------------------------------
.gjrgarchLLH = function(pars, arglist)
{
	# prepare inputs
	...
	ans = try( .C("gjrgarchfilterC", model = as.integer(modelinc[1:21]), 
					pars = as.double(ipars[,1]), idx = as.integer(idx[,1]-1), 
					hEst = as.double(hEst), x = as.double(data), res = as.double(res), 
					nres = double(T), e = double(T), mexdata = mexdata, 
					vexdata = vexdata, zrf = as.double(zrf), constm = double(T), 
					condm = double(T), m = as.integer(m), T = as.integer(T), 
					h = double(T), z = double(T), llh = double(1), 
...
#---------------------------------------------------------------------------------
.gjrgarchfilter = function(spec, data, out.sample = 0, n.old = NULL, rec.init = 'all')
{
...
.ngjrgarchforecast = function(ipars, modelinc, idx, mu, omega, kappa, mxfi, vxfi, h, epsx, z, data, N, n.ahead)
{
	if(modelinc[15]>0){
	...
	for(i in 1:n.ahead){
		...
		if(modelinc[8]>0){
			for (j in 1:modelinc[8]){
				if (i-j > 0){				
					s1 = (h[N + i - j]^2)
					s2 = kappa*ipars[idx["gamma",1]+j-1, 1] * (h[N + i - j]^2)
				} else{
					s1 = epsx[N + i - j]^2
					s2 = ipars[idx["gamma",1]+j-1, 1]*(epsx[N + i - j]^2)*as.integer(epsx[N + i - j]<0)
				}
				h[N+i] = h[N+i] + ipars[idx["alpha",1]+j-1, 1] * s1 + s2
			}
		}
		h[N+i] = sqrt(h[N+i])
	}
		...
		mexsimdata = NULL, vexsimdata = NULL)
{
	...
	for(i in 1:m.sim){
		...
		ans1 = try(.C("gjrgarchsimC", model = as.integer(modelinc[1:21]), 
						pars = as.double(ipars[,1]), idx = as.integer(idx[,1]-1), 
		...
		vexsimdata = NULL)
{
	...
	persist = .persistgjrgarch1(pars = ipars[,1], idx, distribution)		
